...
import crabAnimation from '../../assets/animation/crap.json'; // Add the crab animation
import wheelAnimation from '../../assets/animation/wheel.json'; // Add the wheel animation
import backButtonImage from '../../assets/images/back.png';
import exitButtonImage from '../../assets/images/exit.png';
import restartButtonImage from '../../assets/images/restart.png'; // Add the restart button image
...
import backButtonSound from '../../assets/sound/backBT.wav'; // Import the back button sound
import exitButtonSound from '../../assets/sound/backBT.wav'; // Import the exit button sound
import forwardSound from '../../assets/sound/forward.mp3'; // Import the forward sound
...
const ClipPhotoMatch = () => {
    ...
    useEffect(() => {
        ...
        backAudioRef.current.load(); // Ensure the audio element is loaded
        forwardAudioRef.current = new Audio(forwardSound);
        forwardAudioRef.current.load(); // Ensure the forward audio element is loaded
        exitAudioRef.current = new Audio(exitButtonSound);
        exitAudioRef.current.load(); // Ensure the exit audio element is loaded
        console.log('Audio files loaded');
    }, []);

    useEffect(() => {
        if (shipPosition > 0) { // Ensure it's not playing on initial render
            forwardAudioRef.current.currentTime = 0; // Reset audio to start
    ...
    const handleBackClick = () => {
        if (backAudioRef.current) {
            console.log('Back button clicked, playing sound');
            backAudioRef.current.currentTime = 0; // Reset audio to start
            backAudioRef.current.play().catch(error => {
                console.error('Error playing back button sound:', error);
            }); // Play the back button sound
        } else {
            console.log('Back audio ref is null');
        }
        navigate('/login');
    };
    ...
    const handleExitClick = () => {
        if (exitAudioRef.current) {
            exitAudioRef.current.currentTime = 0; // Reset audio to start
            exitAudioRef.current.play().catch(error => {
                console.error('Error playing exit button sound:', error);
            }); // Play the exit button sound
        } else {
            console.log('Exit audio ref is null');
        }
        navigate('/');
    };
